...
@section('content')
  @include('includes.user_sidebar', ['requestStatus' => $requestStatus])
    ...
    <x-forms.form method="POST" action="/users/products/{{ $product->id }}/edit" enctype="multipart/form-data">
        @csrf
         ...
         <x-forms.input label="Add Product Gallery" name="images[]" type="file" multiple />
           ...
           <div class="grid lg:grid-cols-4 gap-8 mt-6 text-white-400">
             @foreach($product->galleries as $gallery)
                <div>
                 <img src="{{ asset('storage/' . $gallery->image_path) }}" alt="Gallery Image" style="width: 50px; height: auto;">
              </div>
            @endforeach
            </div>
          @else
